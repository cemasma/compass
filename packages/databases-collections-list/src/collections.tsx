...
const CollectionItem: React.FunctionComponent<
  ...
  >
> = ({ item: coll, ...props }) => {
  const { type } = useListedCollection(coll) ?? {};
  const { data: info, status: infoStatus } = useCollectionInfo(coll) ?? {};
  const { data: stats, status: statsStatus } = useCollectionStats(coll) ?? {};
